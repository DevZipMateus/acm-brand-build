...
const Products = () => {
  ...
  return (
    <section id="produtos" className="py-12 sm:py-16 lg:py-20 bg-muted">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-3 sm:mb-4">Nossos produtos</h2>
          ...
          <div className="grid sm:grid-cols-2 gap-4 sm:gap-6 mb-12 sm:mb-16">
            {products.map((product, index) => {
              ...
              return (
                <div
                  key={index}
                  className="bg-card rounded-xl shadow-lg p-6 sm:p-8 hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
                >
                  <div className="w-12 h-12 sm:w-14 sm:h-14 bg-primary/10 rounded-lg flex items-center justify-center mb-4 sm:mb-6">
                    <Icon className="w-6 h-6 sm:w-7 sm:h-7 text-primary" />
                  </div>
                  <h3 className="text-lg sm:text-xl font-bold mb-2 sm:mb-3">{product.title}</h3>
          ...
          <div className="bg-card rounded-xl sm:rounded-2xl shadow-xl p-6 sm:p-8 md:p-10 lg:p-12">
            <h3 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-center">Insumos e complementos</h3>
            <div className="grid sm:grid-cols-2 gap-3 sm:gap-4">
              {supplies.map((supply, index) => (
                <div key={index} className="flex items-start gap-2 sm:gap-3">
                  <div className="w-2 h-2 bg-primary rounded-full mt-1.5 sm:mt-2 flex-shrink-0"></div>
                  <p className="text-sm sm:text-base text-foreground">{supply}</p>
                </div>
              ))}
            </div>
            <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-border text-center">
              ...
              <a
                ...
                className="bg-primary text-primary-foreground px-6 sm:px-8 py-2.5 sm:py-3 rounded-lg hover:bg-primary/90 transition-all duration-300 font-semibold inline-flex items-center gap-2 shadow-md hover:shadow-lg text-sm sm:text-base"
              >
